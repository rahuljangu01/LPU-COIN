...
// --- 1. GMAIL CONFIGURATION (Cloud IPv4 Force Fix) ---
const transporter = nodemailer.createTransport({
  host: 'smtp.gmail.com',
  port: 465,       // Port 465 is usually more stable for SSL on Render
  secure: true,    // true for 465
  auth: {
    user: process.env.EMAIL_USER || 'rahuljangu01@gmail.com', 
    pass: process.env.EMAIL_PASS || 'htjsgoxpzvalgtth' 
  },
  // 🔥 IMPORTANT: Yeh settings IPv6 error ko khatam kar dengi
  connectionTimeout: 10000, 
  greetingTimeout: 10000,
  socketTimeout: 10000,
  dnsServer: '8.8.8.8' // Google DNS use karein
});
...
// --- 2. SEND OTP (Faster & Non-Blocking) ---
export const sendOTP = async (req, res) => {
  try {
    ...
    // 🔥 SUCCESS REPLY TURANT BHEJO (Frontend ko intezar nahi karwayenge)
    // Isse aapka "Stuck" hone wala problem solve ho jayega
    ...
    const mailOptions = {
      ...
      to: email,
      subject: 'LPU COIN - Security Code',
      html: `<div style="font-family: sans-serif; padding: 20px; border: 2px solid #3b82f6;">
              <h2>Security Verification</h2>
              <p>Your security code is:</p>
              <h1 style="background:#f1f5f9; padding:20px; text-align:center; letter-spacing:10px;">${otp}</h1>
            </div>`
    };
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      if (error) {
        console.log("❌ CLOUD EMAIL ERROR:", error.message);
      } else {
        console.log("🚀 EMAIL SENT FROM RENDER:", info.response);
      }
    });
...
// ... baaki register/login functions wahi rakhein
