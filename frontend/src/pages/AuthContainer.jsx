...
export default function AuthContainer() {
  ...
  const handleForgotRequest = async (e) => {
    e.preventDefault();
    setResetStatus({ type: 'loading', msg: 'Verifying Identity...' });
    try {
  ...
  // --- 🚀 ULTRA-FAST MOBILE LOOP SCANNING ---
  const handleBiometricAuth = async () => {
    ...
    try {
      ...
      const scanLoop = setInterval(async () => {
          ...
          if (isLogin) {
            try {
              ...
              if (distance < 0.6) { 
                await login(formData.email, formData.password); 
                stopCamera(); navigate('/'); 
              } else { setError("BIOMETRIC MISMATCH"); stopCamera(); }
            } catch (err) {
        ...
        if (attempts >= 20) { // ~12 seconds timeout
          clearInterval(scanLoop);
          setError("FACE NOT DETECTED - PLEASE RE-SCAN");
          stopCamera();
        }
      }, 600);
    } catch (err) { setError("CAMERA BLOCKED"); setIsScanning(false); }
  };
  ...
  const handleSendOTP = async (e) => {
    ...
    try {
      ...
      if (res.status === 200 || res.data.success) { setRegStep(2); }
    } catch (e) { setError(e.response?.data?.message || "SERVER BUSY... RETRY IN 10s"); }
  };
    ...
    <div className="h-[100dvh] w-screen bg-[#010409] flex items-center justify-center p-6 overflow-hidden font-sans selection:bg-blue-500/30">
        ...
        {isScanning && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-black/95 flex flex-col items-center justify-center p-6 backdrop-blur-xl">
            <div className="relative w-48 h-48 md:w-56 md:h-56 rounded-full border border-blue-500/30 overflow-hidden shadow-2xl">
               ...
               <motion.div animate={{ top: ["0%", "100%", "0%"] }} transition={{ duration: 1.5, repeat: Infinity }} className="absolute w-full h-[1px] bg-blue-400 shadow-[0_0_15px_blue] z-20" />
            </div>
      ...
      <div className="relative w-full max-w-[290px] md:max-w-[700px] md:h-[480px] bg-[#0d1117]/90 rounded-[2.5rem] md:rounded-[3.5rem] border border-white/5 shadow-2xl flex flex-col md:flex-row overflow-hidden z-10 shadow-black/90">
            ...
            <AnimatePresence mode="wait">
              {regStep === 1 && (
                <motion.div key="step1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
                   ...
                   <form onSubmit={handleSendOTP} className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      <input required placeholder={formData.role === 'user' ? "Full Name" : "Shop Name"} className="bg-black/20 p-2 md:p-2.5 text-[9px] rounded-lg border border-white/5 text-white outline-none" onChange={(e)=>setFormData({...formData, name: e.target.value})} />
                      <input required placeholder={formData.role === 'user' ? "Reg ID" : "Vendor ID"} className="bg-black/20 p-2 md:p-2.5 text-[9px] rounded-lg border border-white/5 font-mono text-white outline-none" onChange={(e)=>setFormData({...formData, collegeId: e.target.value})} />
                      ...
                      <input required type="email" placeholder="Email Address" className="bg-black/20 p-2.5 text-[9px] rounded-lg border border-white/5 text-white outline-none" onChange={(e)=>setFormData({...formData, email: e.target.value})} />
              ...
              {regStep === 2 && (
                <motion.div key="step2" initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ opacity: 0 }} className="space-y-4 text-center py-2">
                   <p className="text-[8px] text-slate-500 font-black uppercase tracking-[0.2em]">Verification Protocol</p>
                   ...
                   <button onClick={handleBiometricAuth} className="w-full bg-blue-600 py-3 rounded-lg text-white font-black text-[9px] uppercase tracking-widest shadow-lg">Verify & Scan</button>
              ...
              {regStep === 3 && (
                <motion.form key="step3" initial={{ y: 10 }} animate={{ y: 0 }} onSubmit={async (e)=>{ e.preventDefault(); await register(formData.name, formData.email, formData.password, formData.role, formData.collegeId, formData.phoneNumber, faceDescriptor, formData.otp); navigate('/'); }} className="space-y-3 py-2 text-center">
                   ...
                   <button className="w-full bg-emerald-600 py-3 rounded-lg text-white font-black text-[9px] uppercase tracking-widest shadow-xl">Establish Nexus Identity</button>
                </motion.form>
              )}
            </AnimatePresence>
        </div>
        ...
        <motion.div 
          ...
          className="hidden md:flex absolute top-0 left-0 w-1/2 h-full z-50 bg-[#0d1117] border-x border-[#30363d] flex flex-col items-center justify-center p-8 text-center shadow-2xl shadow-black"
        >
           ...
           <p className="text-[7px] text-slate-500 font-black uppercase tracking-[0.5em] mt-3 italic text-center leading-none">Campus Economy Node</p>
        ...
        {showForgotModal && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-10 bg-black/95 backdrop-blur-md text-center">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-[#0d1117] border border-white/5 p-6 rounded-[2rem] w-full max-w-[240px] shadow-2xl">
              ...
              <input type="email" placeholder="Email Address" className="bg-black/40 p-2.5 text-[9px] rounded-lg border border-white/10 text-white text-center w-full mb-4 outline-none focus:border-blue-500 uppercase tracking-[0.1em]" onChange={(e) => setResetEmail(e.target.value)} />
