...
export default function AuthContainer() {
  ...
  const handleBiometricAuth = async () => {
    if (!modelsLoaded) return alert("AI Systems Booting...");
    setIsScanning(true);
    setError('');
    ...
    try {
      ...
      // ⏳ Camera Stabilizeintezar
      setTimeout(async () => {
        // 🔥 Sensitivity Fix: minConfidence lowered to 0.4
        ...
        if (detection) {
          if (isLogin) {
            try {
              ...
              setError("IDENTITY NOT REGISTERED");
              stopCamera();
            }
          } else {
            setFaceDescriptor(Array.from(detection.descriptor));
            stopCamera(); 
            setRegStep(3); // Move to final password step
          }
        } else { 
          setError("FACE NOT DETECTED - IMPROVE LIGHT"); 
          stopCamera(); 
        }
      }, 3000);
    } catch (err) { 
  ...
  const handleSendOTP = async (e) => {
    ...
    try {
      ...
      const msg = e.response?.data?.message || "SYSTEM BUSY... PLEASE REFRESH";
      setError(msg); 
    }
  };
  ...
  return (
    <div className="h-[100dvh] w-screen bg-[#010409] flex items-center justify-center p-6 overflow-hidden font-sans selection:bg-blue-500/30">
      ...
      {/* --- COMPACT CARD --- */}
      <div className="relative w-full max-w-[290px] md:max-w-[720px] md:h-[500px] bg-[#0d1117]/90 rounded-[2.5rem] md:rounded-[3.5rem] border border-white/5 shadow-2xl flex flex-col md:flex-row overflow-hidden z-10 shadow-black/90">
        ...
        <div className={`w-full md:w-1/2 p-6 md:p-10 flex flex-col justify-center ${!isLogin && 'hidden md:flex'}`}>
            ...
            <form onSubmit={handleLoginAttempt} className="space-y-3 md:space-y-4">
              ...
              <button className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-3 rounded-lg md:rounded-xl text-[8px] md:text-[9px] uppercase tracking-[0.2em] shadow-lg active:scale-95 transition-all mt-1">Authorize Access</button>
            </form>
            <div className="mt-4 text-center md:text-left">
              ...
              <div className="mt-6 md:hidden pt-4 border-t border-white/5 flex flex-col items-center">
                <button onClick={() => setIsLogin(false)} className="text-blue-500 text-[9px] font-black uppercase tracking-widest">Enroll Node <ChevronRight size={10} className="inline ml-1"/></button>
              </div>
            </div>
        </div>
            ...
            <AnimatePresence mode="wait">
              {regStep === 1 && (
                <motion.div key="step1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
                   ...
                   <form onSubmit={handleSendOTP} className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      <input required placeholder={formData.role === 'user' ? "Full Name" : "Shop Name"} className="bg-black/20 p-2.5 text-[9px] rounded-lg border border-white/5 text-white" onChange={(e)=>setFormData({...formData, name: e.target.value})} />
                      <input required placeholder={formData.role === 'user' ? "Reg ID" : "Vendor ID"} className="bg-black/20 p-2.5 text-[9px] rounded-lg border border-white/5 font-mono text-white" onChange={(e)=>setFormData({...formData, collegeId: e.target.value})} />
                      <input required placeholder="Mobile No." className="bg-black/20 p-2.5 text-[9px] rounded-lg border border-white/5 text-white" onChange={(e)=>setFormData({...formData, phoneNumber: e.target.value})} />
                      <input required type="email" placeholder="Email ID" className="bg-black/20 p-2.5 text-[9px] rounded-lg border border-white/5 text-white" onChange={(e)=>setFormData({...formData, email: e.target.value})} />
                      <button className="md:col-span-2 w-full bg-emerald-600 py-3 rounded-lg md:rounded-xl text-white font-black text-[8px] md:text-[9px] uppercase tracking-widest shadow-emerald-900/10 mt-1">Request OTP</button>
                   </form>
              ...
              {regStep === 2 && (
                <motion.div key="step2" initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ opacity: 0 }} className="space-y-4 text-center py-2">
                   ...
                   <button onClick={handleBiometricAuth} className="w-full bg-blue-600 py-3 rounded-lg text-white font-black text-[9px] uppercase">Identify & Scan</button>
              ...
              {regStep === 3 && (
                <motion.form key="step3" initial={{ y: 10 }} animate={{ y: 0 }} onSubmit={async (e)=>{ e.preventDefault(); await register(formData.name, formData.email, formData.password, formData.role, formData.collegeId, formData.phoneNumber, faceDescriptor, formData.otp); navigate('/'); }} className="space-y-3 py-2 text-center">
                   <div className="bg-emerald-500/5 p-3 rounded-xl border border-emerald-500/10 flex items-center justify-center gap-3">
                      ...
                      <p className="text-[9px] font-black text-white uppercase tracking-tighter italic leading-none">Identity Secured</p>
                   </div>
        ...
        <motion.div animate={{ x: isLogin ? '100%' : '0%' }} transition={{ type: 'spring', stiffness: 120, damping: 20 }} className="hidden md:flex absolute top-0 left-0 w-1/2 h-full z-50 bg-[#0d1117] border-x border-[#30363d] flex flex-col items-center justify-center p-8 text-center shadow-2xl">
           ...
           <p className="text-[7px] text-slate-500 font-black uppercase tracking-[0.5em] mt-3 italic text-center leading-none">Campus Economy Protocol</p>
