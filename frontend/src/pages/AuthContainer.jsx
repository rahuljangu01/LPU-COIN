      ...
      <AnimatePresence>
        {isScanning && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-black/95 flex flex-col items-center justify-center p-6 backdrop-blur-xl">
            <div className="relative w-48 h-48 md:w-56 md:h-56 rounded-full border border-blue-500/30 overflow-hidden shadow-2xl shadow-blue-500/20">
      ...
      <div className="relative w-full max-w-[290px] md:max-w-[700px] md:h-[480px] bg-[#0d1117]/90 rounded-[2.5rem] md:rounded-[3.5rem] border border-white/5 shadow-2xl flex flex-col md:flex-row overflow-hidden z-10">
        ...
        <div className={`w-full md:w-1/2 p-6 md:p-10 flex flex-col justify-center ${!isLogin && 'hidden md:flex'}`}>
            ...
            <form onSubmit={handleLoginAttempt} className="space-y-3 md:space-y-4">
              <div className="relative flex items-center bg-black/40 border border-white/5 md:border-white/10 rounded-lg md:rounded-xl focus-within:border-blue-500/40 transition-all">
                <Mail className="absolute left-3 md:left-4 text-slate-700" size={12}/>
              ...
              <div className="relative flex items-center bg-black/40 border border-white/5 md:border-white/10 rounded-lg md:rounded-xl focus-within:border-blue-500/40 transition-all">
                <Lock className="absolute left-3 md:left-4 text-slate-700" size={12}/>
                <input required type="password" placeholder="Password" className="w-full bg-transparent p-2.5 md:p-3 pl-9 md:pl-10 text-[9px] md:text-[10px] font-bold text-white outline-none" onChange={(e)=>setFormData({...formData, password: e.target.value})} />
              </div>
              <button className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-2.5 md:py-3.5 rounded-lg md:rounded-xl text-[8px] md:text-[9px] uppercase tracking-widest shadow-lg">Authorize Access</button>
            </form>
            <div className="mt-4 text-center md:text-left">
              ...
              <div className="mt-4 md:hidden pt-4 border-t border-white/5 flex flex-col items-center">
                <button onClick={() => setIsLogin(false)} className="text-blue-500 text-[9px] font-black uppercase tracking-widest">Enroll Node <ChevronRight size={10} className="inline"/></button>
              </div>
            </div>
        </div>
        ...
        <div className={`w-full md:w-1/2 p-6 md:p-10 flex flex-col justify-center bg-[#0d1117] border-l border-white/5 ${isLogin && 'hidden md:flex'}`}>
            <h2 className="text-sm md:text-2xl font-black text-white italic uppercase mb-4 md:mb-6 text-emerald-500 text-center md:text-left">En <span className="text-white font-normal">Roll</span></h2>
            <AnimatePresence mode="wait">
              {regStep === 1 && (
                <motion.div key="step1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
                   <div className="flex bg-black/40 p-0.5 md:p-1 rounded-lg md:rounded-xl border border-white/5 mb-2">
                      <button onClick={()=>setFormData({...formData, role:'user'})} className={`flex-1 py-1.5 md:py-2 text-[8px] md:text-[9px] font-black uppercase rounded-md ${formData.role === 'user' ? 'bg-emerald-600 text-white' : 'text-slate-600'}`}>Student</button>
                      <button onClick={()=>setFormData({...formData, role:'merchant'})} className={`flex-1 py-1.5 md:py-2 text-[8px] md:text-[9px] font-black uppercase rounded-md ${formData.role === 'merchant' ? 'bg-emerald-600 text-white' : 'text-slate-600'}`}>Vendor</button>
                   </div>
                   <form onSubmit={handleSendOTP} className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      ...
                      <button className="md:col-span-2 w-full bg-emerald-600 py-3 rounded-lg md:rounded-xl text-white font-black text-[8px] md:text-[9px] uppercase tracking-widest shadow-emerald-900/10 mt-1 shadow-lg">Request OTP</button>
                   </form>
              ...
              {regStep === 2 && (
                <motion.div key="step2" initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ opacity: 0 }} className="space-y-4 text-center py-2">
                   ...
                   <button onClick={handleBiometricAuth} className="w-full bg-blue-600 py-3 rounded-lg text-white font-black text-[9px] uppercase">Verify & Scan Face</button>
              ...
              {regStep === 3 && (
                <motion.form key="step3" initial={{ y: 10 }} animate={{ y: 0 }} onSubmit={async (e)=>{ e.preventDefault(); await register(formData.name, formData.email, formData.password, formData.role, formData.collegeId, formData.phoneNumber, faceDescriptor, formData.otp); navigate('/'); }} className="space-y-3 py-2 text-center">
                   ...
                   <button className="w-full bg-emerald-600 py-3 rounded-lg text-white font-black text-[9px] uppercase shadow-xl">Establish Nexus Identity</button>
                </motion.form>
              )}
            </AnimatePresence>
        </div>
        ...
        <motion.div animate={{ x: isLogin ? '100%' : '0%' }} transition={{ type: 'spring', stiffness: 120, damping: 20 }} className="hidden md:flex absolute top-0 left-0 w-1/2 h-full z-50 bg-[#0d1117] border-x border-[#30363d] flex flex-col items-center justify-center p-8 text-center shadow-2xl">
           ...
           <p className="text-[7px] text-slate-500 font-black uppercase tracking-[0.5em] mt-3 italic text-center leading-none">Distributed Campus Economy Node</p>
