...
export default function AuthContainer() {
  ...
  const [regStep, setRegStep] = useState(1); // 1: Details, 2: OTP, 3: Password
  ...
  useEffect(() => {
    const loadModels = async () => {
      ...
      } catch (e) { console.error("AI Systems Offline"); }
    };
    loadModels();
  }, []);
  ...
  const handleForgotRequest = async (e) => {
    ...
    try {
      ...
      setResetStatus({ type: 'success', msg: 'Key Dispatched! ✅' });
      setTimeout(() => { setShowForgotModal(false); setResetStatus({ type: '', msg: '' }); }, 2000);
    } catch (err) { setResetStatus({ type: 'error', msg: 'Identity Not Found' }); }
  };

  // --- 🚀 MOBILE OPTIMIZED LOOP SCANNING ---
  const handleBiometricAuth = async () => {
    ...
    try {
      ...
      let attempts = 0;
      const maxAttempts = 20; // 20 baar scan karne ki koshish karega
      ...
      const scanLoop = setInterval(async () => {
        ...
        const detection = await faceapi.detectSingleFace(
          videoRef.current, 
          new faceapi.SsdMobilenetv1Options({ minConfidence: 0.3 }) // High sensitivity
        ...
        if (detection) {
          ...
          if (isLogin) {
            try {
              ...
              setError(err.response?.status === 404 ? "ACCOUNT NOT FOUND" : "SYNC ERROR");
              stopCamera();
            }
          } else {
        ...
        if (attempts >= maxAttempts) {
          clearInterval(scanLoop);
          setError("FACE NOT DETECTED - PLEASE KEEP STEADY");
          stopCamera();
        }
      }, 600); // 0.6 second gap between scans
    ...
    <div className="h-[100dvh] w-screen bg-[#010409] flex items-center justify-center p-6 overflow-hidden font-sans">
      ...
      <AnimatePresence>
        {isScanning && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-black/95 flex flex-col items-center justify-center p-6 backdrop-blur-xl">
            ...
            <h2 className="text-white font-black tracking-widest mt-6 uppercase text-[8px] animate-pulse italic text-center">Identifying Nexus Node...</h2>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="relative w-full max-w-[290px] md:max-w-[700px] md:h-[480px] bg-[#0d1117]/90 rounded-[2.5rem] md:rounded-[3.5rem] border border-white/5 shadow-2xl flex flex-col md:flex-row overflow-hidden z-10 shadow-black/90">
        ...
        <div className={`w-full md:w-1/2 p-6 md:p-10 flex flex-col justify-center ${!isLogin && 'hidden md:flex'}`}>
            ...
            <div className="mt-4 text-center md:text-left flex flex-col items-center md:items-start gap-3">
              ...
              <div className="mt-4 md:hidden pt-4 border-t border-white/5 flex flex-col items-center">
                <button onClick={() => setIsLogin(false)} className="text-blue-500 text-[9px] font-black uppercase tracking-widest flex items-center gap-1">Enroll New Node <ChevronRight size={10}/></button>
              </div>
            </div>
        </div>
            ...
            <AnimatePresence mode="wait">
              {regStep === 1 && (
                <motion.div key="step1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
                   ...
                   <form onSubmit={handleSendOTP} className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      ...
                      <input required type="email" placeholder="Email ID" className="bg-black/20 p-2 md:p-2.5 text-[9px] rounded-lg border border-white/5 text-white outline-none" onChange={(e)=>setFormData({...formData, email: e.target.value})} />
              ...
              {regStep === 2 && (
                <motion.div key="step2" initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ opacity: 0 }} className="space-y-4 text-center py-2">
                   ...
                   <button onClick={handleBiometricAuth} className="w-full bg-blue-600 py-3 rounded-lg text-white font-black text-[9px] uppercase tracking-widest">Verify & Scan Face</button>
        ...
        <motion.div animate={{ x: isLogin ? '100%' : '0%' }} transition={{ type: 'spring', stiffness: 120, damping: 20 }} className="hidden md:flex absolute top-0 left-0 w-1/2 h-full z-50 bg-[#0d1117] border-x border-[#30363d] flex flex-col items-center justify-center p-8 text-center shadow-2xl">
           ...
           <p className="text-[7px] text-slate-500 font-black uppercase tracking-[0.5em] mt-3 italic text-center">Campus Economy Protocol</p>
